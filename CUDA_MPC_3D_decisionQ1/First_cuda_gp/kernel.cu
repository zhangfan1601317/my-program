//在MPC模型中将h改为Q,拉速保持不变1.2m/min,先运行到稳态的，再加入约束
...
# define StaticIter 900
# define M 3
# define N M+2*CoolSection
# define TestIter 2000
# define limit 2001//limit>=tnpts/num_iter
...
float Taim[CoolSection] = { 966.149841, 925.864746, 952.322083, 932.175537, 914.607117, 890.494263, 870.804443, 850.595825 };
...
float calculateLiquidLineLength(float *T_result, int nx, int ny, int nz, float Ts, float length);
...
int main()
{
	...
	int tnpts = 20001;  // time step
	float  Lx = 0.25, Ly = 28.599, Lz = 0.25, t_final = 4000.0, dx, dy, dz, tao;  //T_Cast = 1558.0, T_Cast is the casting temperature Lx and Ly is the thick and length of steel billets
	...
	for (int m = 0; m < nz; m++)
		for (int j = 0; j < ny; j++)
	       for (int i = 0; i < nx; i++)
			   T_Init[nx * ny * m + j * nx + i] = 1558.0f;  // give the initial condition
...
cudaError_t addWithCuda(float *T_Init, float dx, float dy, float dz, float tao, int nx, int ny, int nz, int tnpts, int num_blocks, int num_threadsx, int num_threadsy)
{
	...
	float AveSurfaceTemperature[limit] = { 0 };
	float LiquidLine[limit] = { 0 };
	...
	{			
		...
		for (int temp = 0; temp < Section; temp++)
		{
			...
			for (int temp = 0; temp < CoolSection; temp++) {
				...
				Sum_Mean_Tsurface += Mean_TSurface[temp+MoldSection];
			}
			AveSurfaceTemperature[t/Num_Iter] = Sum_Mean_Tsurface / CoolSection;
			LiquidLine[t / Num_Iter] = calculateLiquidLineLength(T_Result, nx, ny, nz, Ts, 28.599f);
	...
	ofstream fout;
		fout.open("F:\\data_zf\\GPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "GPUMPC3D2block3threads is not open" << endl;
		else
		{
			for (int j = 0; j < ny; j++)
			{
		...
		fout.open("F:\\data_zf\\SurfaceGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "SurfaceGPUMPC3D2block3threads is not open" << endl;
		else 
		{
			for (int j = 0; j < ny; j++)
			{
		...
		fout.open("F:\\data_zf\\CenterGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "CenterGPUMPC3D2block3threads is not open" << endl;
		else
		{
			for (int j = 0; j < ny; j++)
			{
		...
		fout.open("F:\\data_zf\\SurfaceErrorGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "SurfaceErrorGPUMPC3D2block3threads is not open" << endl;
		else
		{
		...
		fout.open("F:\\data_zf\\ThicknessGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "ThicknessGPUMPC3D2block3threads is not open" << endl;
		else
		{
			for (int i = 0; i < CoolSection; i++) {
		...
		fout.open("F:\\data_zf\\lamda.txt");
		if (!fout)
			cout << "lamda is not open" << endl;
		else
		{
		...
		fout.open("F:\\data_zf\\gtest.txt");
		if (!fout)
			cout << "gtest is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\htest.txt");
		if (!fout)
			cout << "htest is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\fitnesstest.txt");
		if (!fout)
			cout << "fitnesstest is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\c.txt");
		if (!fout)
			cout << "c is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\step.txt");
		if (!fout)
			cout << "step is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\AveSurfaceTemperature.txt");
		if (!fout)
			cout << "AveSurfaceTemperature is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
				fout << AveSurfaceTemperature[i] << ",";
		...
		fout.open("F:\\data_zf\\LiquidLine.txt");
		if (!fout)
			cout << "LiquidLine is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
...
float calculateLiquidLineLength(float *T_result, int nx, int ny, int nz,float Ts, float length) {
	float line = 0;
	for (int j = 0; j < ny; j++) {
		if (T_result[nx * nz * j + (nx / 2)* nz + int((nx - 1) / 2)] < Ts ) {
			line = j*length / 3000.0f;
			break;
		}
	}
	//printf(" line=%f \n", line);
