//在MPC模型中将h改为Q,修改h观察迭代过程,减少h的初始值
...
float H_Init[Section] = { 1300,1100,900,700,1100,650,200,200,250,200,150,50 };
...
cudaError_t addWithCuda(float *T_Init, float dx, float dy, float dz, float tao, int nx, int ny, int nz, int tnpts, int num_blocks, int num_threadsx, int num_threadsy)
{
	...
	ofstream fout;
		fout.open("F:\\data_zf\\low_hGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "low_hGPUMPC3D2block3threads is not open" << endl;
		else
		{
			for (int j = 0; j < ny; j++)
			{
		...
		fout.open("F:\\data_zf\\low_hSurfaceGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "low_hSurfaceGPUMPC3D2block3threads is not open" << endl;
		else 
		{
			for (int j = 0; j < ny; j++)
			{
		...
		fout.open("F:\\data_zf\\low_hCenterGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "low_hCenterGPUMPC3D2block3threads is not open" << endl;
		else
		{
			for (int j = 0; j < ny; j++)
			{
		...
		fout.open("F:\\data_zf\\low_hSurfaceErrorGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "low_hSurfaceErrorGPUMPC3D2block3threads is not open" << endl;
		else
		{
		...
		fout.open("F:\\data_zf\\low_hThicknessGPUMPC3D2block3threads.txt");
		if (!fout)
			cout << "low_hThicknessGPUMPC3D2block3threads is not open" << endl;
		else
		{
			for (int i = 0; i < CoolSection; i++) {
		...
		fout.open("F:\\data_zf\\low_hlamda.txt");
		if (!fout)
			cout << "low_hlamda is not open" << endl;
		else
		{
		...
		fout.open("F:\\data_zf\\low_hgtest.txt");
		if (!fout)
			cout << "low_hgtest is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\low_hhtest.txt");
		if (!fout)
			cout << "low_hhtest is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\low_hfitnesstest.txt");
		if (!fout)
			cout << "low_hfitnesstest is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\low_hc.txt");
		if (!fout)
			cout << "low_hc is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\low_hstep.txt");
		if (!fout)
			cout << "low_hstep is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
