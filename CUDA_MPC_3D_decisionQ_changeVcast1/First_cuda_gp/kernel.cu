...
int main()
{
	...
	float T_Cast = 1558.0, Lx = 0.25, Ly = 28.599, Lz = 0.25, t_final = 4000.0, dx, dy, dz, tao;  // T_Cast is the casting temperature Lx and Ly is the thick and length of steel billets
	...
	for (int m = 0; m < nz; m++)
		for (int j = 0; j < ny; j++)
	       for (int i = 0; i < nx; i++)
			   T_Init[nx * ny * m + j * nx + i] = 1558;  // give the initial condition
...
cudaError_t addWithCuda(float *T_Init, float dx, float dy, float dz, float tao, int nx, int ny, int nz, int tnpts, int num_blocks, int num_threadsx, int num_threadsy)
{
	...
	{			
		if (t / Num_Iter >= (100+StaticIter))//1000之后
			Vcast = -0.017;
		if (t % Num_Iter == 0)
		{
			int iter = t / Num_Iter;
		...
		for (int temp = 0; temp < Section; temp++)
		{
			...
			//printf(" LiquidLine= %f", LiquidLine[t / Num_Iter]);
			ofstream fout;
			if (t == 9000) 
			{
				
				fout.open("F:\\data_zf\\changeVcastSurTGPUMPC.txt");
				if (!fout)
					cout << "changeVcastSurTGPUMPC is not open" << endl;
				else
				{
					for(int j = 0; j < ny; j++)
					{
			...
			if (t == 10600)
			{
				fout.open("F:\\data_zf\\changeVcastSurTGPUMPCTwoMinutes.txt");
				if (!fout)
					cout << "changeVcastSurTGPUMPCTwoMinutes is not open" << endl;
				else
				{
					for (int j = 0; j < ny; j++)
					{
			...
			if(t==10300)
			{
				fout.open("F:\\data_zf\\changeVcastSurTGPUMPCOneMinute.txt");
				if (!fout)
					cout << "changeVcastSurTGPUMPCOneMinute is not open" << endl;
				else
				{
					for (int j = 0; j < ny; j++)
					{
			...
			if (t == 11500)
			{
				fout.open("F:\\data_zf\\changeVcastSurTGPUMPCFiveMinutes.txt");
				if (!fout)
					cout << "changeVcastSurTGPUMPCFiveMinutes is not open" << endl;
				else
				{			
					for (int j = 0; j < ny; j++)
					{
			...
			if (t == 13000) 
			{
				fout.open("F:\\data_zf\\changeVcastSurTGPUMPCTenMinutes.txt");
				if (!fout)
					cout << "changeVcastSurTGPUMPCTenMinutes is not open" << endl;
				else
				{
					for (int j = 0; j < ny; j++)
					{
			...
			if (t == 16000) 
			{
				fout.open("F:\\data_zf\\changeVcastSurTGPUMPCTwentyMinutes.txt");
				if (!fout)
					cout << "changeVcastSurTGPUMPCTwentyMinutes is not open" << endl;
				else
				{
					for (int j = 0; j < ny; j++)
					{
			...
			if (t == 19000)
			{
				fout.open("F:\\data_zf\\changeVcastSurTGPUMPCThirtyMinutes.txt");
				if (!fout)
					cout << "changeVcastSurTGPUMPCThirtyMinutes is not open" << endl;
				else
				{
					for (int j = 0; j < ny; j++)
					{
	...
	ofstream fout;
		...
		fout.open("F:\\data_zf\\changeVcastAveSurfaceTemperature.txt");
		if (!fout)
			cout << "changeVcastAveSurfaceTemperature is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
		...
		fout.open("F:\\data_zf\\changeVcastLiquidLine.txt");
		if (!fout)
			cout << "changeVcastLiquidLine is not open" << endl;
		else
		{
			for (int i = 0; i < TestIter; i++)
			{
